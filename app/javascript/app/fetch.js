...
function fetchHelper(URL, method, body, success, failure = defaultFailure) {
  fetch(URL, {
    ...
    .then(function(response) {
      if (response.ok) return response.json().then(resp => success(resp));
...
function onFetchApplicationsSuccess(resp) {
  ...
  resp.forEach(app => {
    ...
    else applications[app.applicant_id] = [newApp];
  });
...
function onFetchAssignmentsSuccess(resp) {
  ...
  resp.forEach(ass => {
    ...
    else assignments[ass.applicant_id] = [newAss];
