...
const ApplicantRow = props =>
  <tr key={"applicant-" + props.id + "-row"}>
    {props.config.map((field, i) =>
      <td key={"applicant-" + props.id + "-row-" + i}>
        {field.data(props)}
      </td>
    )}
  </tr>;
...
class ABCApplicantTable extends React.Component {
  ...
  filterApplicants() {
    ...
    if (this.props.assigned) {
      this.applicants = this.props.func.getApplicantsAssignedToCourse(
        this.props.course
      );
    } else {
      this.applicants = this.props.func.getApplicantsToCourseUnassigned(
        this.props.course
      );

      // apply additional filtering and sorting to unassigned applicants
      let panelFields = this.props.func.getCoursePanelFieldsByCourse(
        this.props.course
      );

      this.applicants.sort((a, b) =>
        this.sortApplicants(a, b, panelFields.activeSortFields)
      );
    }
  }
  ...
  render() {
    if (!this.applicants) return null;
