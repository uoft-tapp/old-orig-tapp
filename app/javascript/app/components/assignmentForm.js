...
class AssignmentForm extends React.Component {
    ...
    if (assignments !== undefined) {
      return assignments.map((assignment, index) =>
        <AssignmentRow
          ...
          courses={courses}
          temp={false}
          input_func={this.detectAssignmentHour}
          self={this}
          {...this}
        />
      );
  ...
  noAssignments(assignments, temp_assignments) {
    if (assignments === undefined && temp_assignments === undefined)
      ...
      if (temp_assignments !== undefined) {
        if (temp_assignments.length == 0) return true;
        else {
          return false;
        }
      } else {
      ...
      if (assignments.length == 0 && temp_assignments == 0) return true;
      else {
        return false;
      }
    } else {
    ...
    if (temp_assignments !== undefined) {
      return temp_assignments.map((assignment, index) =>
        <AssignmentRow
          ...
          courses={courses}
          temp={true}
          input_func={this.detectTempAssignmentHour}
          self={this}
          {...this}
        />
      );
  ...
  existingAssignment(positionId, assignments, temp_assignments) {
    for (let assignment in assignments) {
      if (assignments[assignment].positionId == positionId) return true;
    }
    for (let assignment in temp_assignments) {
      if (temp_assignments[assignment].positionId == positionId) return true;
    }
    return false;
  }

  setCourses(courses) {
    return Object.entries(courses).map(key => <option value={key[1].code} />);
  }
